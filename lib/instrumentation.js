...
class LightMyRequestInstrumentation extends InstrumentationBase {
  ...
  _patch (moduleExports, moduleVersion) {
    ...
    // Guard against double patching - check if already patched
    if (moduleExports?.[kOriginal] || moduleExports?.default?.[kOriginal]) {
      this._diag.debug(`${MODULE_NAME}@${moduleVersion} already patched, skipping`)
  ...
  _patchInject () {
    ...
    return (original) => {
      return function patchedInject (dispatchFunc, options, callback) {
        ...
        const span = instrumentation.tracer.startSpan(`${method} ${urlPath}`, {
